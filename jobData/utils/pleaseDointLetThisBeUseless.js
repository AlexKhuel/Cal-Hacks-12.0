...
export function extractAndAnalyzeSkills(jobs, groqRequiredSkills) {
  ...
  Object.entries(jobs).forEach(([jobId, job]) => {
    ...
    const category =
      job.category || job.jobCategory || job.department || "General";
    ...
    if (department)
      jobDepartments[department] = (jobDepartments[department] || 0) + 1;
      ...
      if (Array.isArray(source)) {
        source.forEach((skill) => {
          if (skill && typeof skill === "string" && skill.trim()) {
            ...
            if (!skillContexts[normalizedSkill])
              skillContexts[normalizedSkill] = [];
    ...
    Object.keys(groqRequiredSkills).forEach((skill) => {
      ...
      if (textFields.includes(normalizedSkill)) {
        ...
        if (!skillContexts[normalizedSkill])
          skillContexts[normalizedSkill] = [];
        if (
          !skillContexts[normalizedSkill].some((ctx) => ctx.jobId === jobId)
        ) {
          skillContexts[normalizedSkill].push({
            jobId,
            jobTitle: job.title || job.name || job.position,
  ...
  console.log(
    `- Skills from Groq analysis: ${Object.keys(groqRequiredSkills).length}`
  );
  console.log(
    `- Skills found in job data: ${
      enhancedSkills.filter((s) => s.frequency > 0).length
    }`
  );
...
export function buildAssessmentResult(enhancedSkills, skillLevels) {
  ...
  enhancedSkills.forEach(
    ({ skill, frequency, type, importance, relatedSkills, contexts }) => {
      const level = skillLevels[skill] || skillLevels[skill.toLowerCase()] || 0;
...
export function calculateStatistics(assessedSkills) {
  ...
  const skillsUserHas = Object.values(assessedSkills).filter(
    (s) => s.proficiencyLevel > 0
  ).length;
  const criticalSkillsMatched = Object.values(assessedSkills).filter(
    (s) => s.importance === "critical" && s.proficiencyLevel >= 50
  ).length;
  const criticalSkillsTotal = Object.values(assessedSkills).filter(
    (s) => s.importance === "critical"
  ).length;

  const skillGaps = Object.entries(assessedSkills)
    .filter(
      ([, data]) => data.proficiencyLevel === 0 && data.importance !== "low"
    )
    .sort((a, b) => {
      const importanceWeight = { critical: 3, high: 2, medium: 1 };
      return (
        (importanceWeight[b[1].importance] || 1) -
        (importanceWeight[a[1].importance] || 1)
      );
    });
...
export function displaySkillsSummary(enhancedSkills, groqRequiredSkills) {
  ...
  enhancedSkills
    ...
    .forEach(({ skill, frequency, type, importance }) => {
      console.log(
        `  - ${skill}: ${frequency} jobs, ${type}, ${importance} importance`
      );
    });
}
...
export function displayStatistics(stats) {
  ...
  console.log(
    `- Skills user has experience with: ${stats.skillsUserHas} (${stats.matchPercentage}%)`
  );
  console.log(
    `- Critical skills matched: ${stats.criticalSkillsMatched}/${stats.criticalSkillsTotal}`
  );

  console.log(`\nTop 10 Skill Gaps (required skills user lacks):`);
  stats.skillGaps.slice(0, 10).forEach(([skill, data]) => {
    console.log(
      `  - ${skill}: ${data.importance} importance, needed by ${data.frequency} jobs`
    );
  });
