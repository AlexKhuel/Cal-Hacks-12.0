    ...
    try {
        ...
        for (let i = 0; i < batches.length; i++) {
            ...
            if ((i + 1) % 10 === 0 || i === batches.length - 1) {
                ...
                console.log(
                    `Progress: ${processed}/${totalJobs} jobs processed (${Math.round(
                        (processed / totalJobs) * 100
                    )}%) - Success: ${successfulBatches}, Failed: ${failedBatches}`
                );
            }
        ...
        console.log(
            `Processed: ${
                Object.keys(allWeights).length
            } out of ${totalJobs} jobs`
        );
