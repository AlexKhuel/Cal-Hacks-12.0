            ...
            if (Array.isArray(source)) {
                source.forEach((skill) => {
                    ...
                    skillFrequency[normalized] =
                        (skillFrequency[normalized] || 0) + 1;
                    if (!skillContexts[normalized])
                        skillContexts[normalized] = [];
                    skillContexts[normalized].push({
                        jobId,
                        source: "structured",
                    });
                });
...
export function getTopSkills(skills, limit = 20) {
    return skills.sort((a, b) => b.frequency - a.frequency).slice(0, limit);
}
